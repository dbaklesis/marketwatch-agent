...
async def parse_single_card_async(card_data: dict, site_name: str, client: httpx.AsyncClient) -> ProductItem:
    """Parses card using DOM fields first, falling back to clean LLM instructions."""
    ...
    # Direct DOM hit (bypasses LLM completely)
    ...
    # Fallback LLM prompt with safe neutral placeholders
    prompt = f"""Extract product details from this snippet:
"{raw_text}"

STRICT RULES:
- Do NOT translate text to another language.
- Do NOT copy instructions or template words into fields.
- If site is BooksToScrape, set "author" to "N/A".
    ...
    async with LLM_SEMAPHORE:
        try:
            ...
            if isinstance(parsed, dict):
                p_title = str(parsed.get("title", "")).strip()
                # Safeguard against model echoing prompt placeholders
                if not p_title or p_title in ["Title", "Verbatim EXACT Title"]:
                    return None
